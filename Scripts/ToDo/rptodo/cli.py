...
#Add Sort Command
@app.command(name="list-sorted")
def list_sorted_todos() -> None:
    """List all to-dos sorted by priority."""
    todoer = get_todoer()
    sorted_todos = todoer.get_todo_list_sorted()
    if not sorted_todos:
        typer.echo("No to-dos found.")
        return
    typer.secho("\nSorted To-Do List:\n", fg=typer.colors.BLUE, bold=True)
    columns = (
        "ID.  ",
    ...
    typer.secho(headers, fg=typer.colors.BLUE, bold=True)
    typer.secho("-" * len(headers), fg=typer.colors.BLUE)
    for id, todo in enumerate(sorted_todos, start=1):
        desc, priority, done = todo.values()
        typer.secho(
            f"{id}{(len(columns[0]) - len(str(id))) * ' '}"
            f"| ({priority}){(len(columns[1]) - len(str(priority)) - 4) * ' '}"
